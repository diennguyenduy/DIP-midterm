# Run to open camera
# Enter input to take a picture with filter
...
while(True):
    ...
    # Filtering image by call filter function
    hue_sat = apply_hue_saturation(frame.copy(), alpha=3, beta=3)

    sepia = apply_sepia(frame.copy(), intensity=.8)

    color_overlay = apply_color_overlay(
        frame.copy(), intensity=.8, red=123, green=231)
