...
while(True):
    ...
    if num == '1':
        ...
        # starting pixel coordinates (topleft,of cropping rectangles)
        start_row, start_col = int(height*0.10), int(width*0.10)
        # ending pixel coordinates (bottom right),this can be changed
        end_row, end_col = int(height*0.86), int(width*0.86)
        ...
        resized = cv2.resize(
            img, (int(img.shape[1]*1.5), int(img.shape[0]*1.5)))
        cv2.imshow('resize', resized)
        img1 = resized
    elif num == '14':
        # this will covert the image of horizontal pixel elements into vertical pixel elements as in matrix
